...
bool CSocekt::Initialize_subproc()
{
    ...
    if(pthread_mutex_init(&m_childserverListMutex, NULL)  != 0)
    {        
        ngx_log_stderr(0,"CSocekt::Initialize()中pthread_mutex_init(&m_childserverListMutex)失败.");
        return false;    
    }
    if(pthread_mutex_init(&m_allchildtolistMutex, NULL)  != 0)
    {        
        ngx_log_stderr(0,"CSocekt::Initialize()中pthread_mutex_init(&m_allchildtolistMutex)失败.");
        return false;    
    }
    if(pthread_mutex_init(&m_allchildidtolistMutex, NULL)  != 0)
    {        
        ngx_log_stderr(0,"CSocekt::Initialize()中pthread_mutex_init(&m_allchildidtolistMutex)失败.");
        return false;    
    }
    ...
    if(g_servertype==1)
    {
        ThreadItem *preconnectconn; 
        m_threadVector.push_back(preconnectconn = new ThreadItem(this)); 
        err = pthread_create(&preconnectconn->_Handle, NULL, ServerReconnectChildServer,preconnectconn);
        if(err != 0)
        {
            return false;
        }
...
void CSocekt::Shutdown_subproc()
{
    ...
    pthread_mutex_destroy(&m_childserverListMutex); 
    
    pthread_mutex_destroy(&m_allchildtolistMutex); 
    pthread_mutex_destroy(&m_allchildidtolistMutex); 
...
int CSocekt::ngx_connectto_child_server()
{
    CConfig *p_config = CConfig::GetInstance(); //单例类
    int _childServerNumber = p_config->GetIntDefault("ChildServerNumber",0);//
    for(int i = 0; i < _childServerNumber; i++)
    {
        /* code */
        char _serveripkey[64];
        char _serverportkey[64];
        char _servertypekey[64];
        sprintf(_servertypekey,"ChildServerType%d",i);
        sprintf(_serverportkey,"ChildServerPort%d",i);
        sprintf(_serveripkey,"ChildServerIp%d",i);
        strcat(_servertypekey,"\0");
        strcat(_serverportkey,"\0");
        strcat(_serveripkey,"\0");
        const char* _serverip = p_config->GetString(_serveripkey);
        int _serverport = p_config->GetIntDefault(_serverportkey,1);
        int _servertype = p_config->GetIntDefault(_servertypekey,1);
        ngx_log_stderr(0,"_serverip:%s,_serverport:%d,_servertype:%d\n",_serverip,_serverport,_servertype);
        if(_serverip==nullptr||_serverport==1||_servertype==1)
        {
            ngx_log_stderr(0,"_serverip:%s,_serverport:%d,_servertype:%d\n",_serverip,_serverport,_servertype);
            return -1;
        }
        ...
        lpngx_connection_t p_cnn = ngx_get_connection(sockfd);
        p_cnn->servertype=_servertype;
...
{    
    ...
    while(g_stopEvent == 0) //不退出
    {
        ...
        //ngx_log_stderr(0,"aaaaa!");
        //一般走到这里都表示需要处理数据收发了
        if(g_stopEvent != 0)  //要求整个进程退出
            break;
        //ngx_log_stderr(0,"1111111111111111111111\n");
        if(pSocketObj->m_iSendMsgQueueCount > 0) //原子的 
        {
            ...
            while(pos != posend)
            {
                ...
                {
                    //ngx_log_stderr(0,"hahahhahahahahahhahah\n");
                ...
                //ngx_log_stderr(0,"444444444444444444444444\n");                
                ...
                if(sendsize > 0)
                {                    
                    //ngx_log_stderr(0,"6666666666666666666666666\n");
