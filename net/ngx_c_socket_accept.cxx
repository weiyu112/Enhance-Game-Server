...
void CSocekt::ngx_event_accept(lpngx_connection_t oldc)
{
    ...
    struct sockaddr_in    mysockaddr;        //远端服务器的socket地址
    socklen_t          socklen;
    int                err;
    int                level;
    int                s;
    ...
    {     
        if(use_accept4)
        {
            ...
            s = accept4(oldc->fd, (struct sockaddr*)&mysockaddr, &socklen, SOCK_NONBLOCK); //从内核获取一个用户端连接，最后一个参数SOCK_NONBLOCK表示返回一个非阻塞的socket，节省一次ioctl【设置为非阻塞】调用
        }
        else
        {
            //以为listen套接字是非阻塞的，所以即便已完成连接队列为空，accept()也不会卡在这里；
            s = accept(oldc->fd,  (struct sockaddr*)&mysockaddr, &socklen);
        }
...
void CSocekt::ngx_build_connoction(lpngx_connection_t pConn)
{
    pConn->rhandler = &CSocekt::ngx_read_request_handler;  //设置数据来时的读处理函数，其实官方nginx中是ngx_http_wait_request_handler()
    pConn->whandler = &CSocekt::ngx_write_request_handler; //设置数据发送时的写处理函数。
