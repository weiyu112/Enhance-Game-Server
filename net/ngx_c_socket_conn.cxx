...
void* CSocekt::ServerReconnectChildServer(void *threadData)
{
    ThreadItem *pThread = static_cast<ThreadItem*>(threadData);
    CSocekt *pSocketObj = pThread->_pThis;
    int err;
    int _sleeptime = 1000*1000*5;
    while(1)
    {
        //ngx_log_stderr(0,"重连线程");
        if(pSocketObj->m_childFreeServerConnectList.size()>0)
        {
            //ngx_log_stderr(0,"开始重连");
            err = pthread_mutex_unlock(&pSocketObj->m_childserverListMutex); 
            if(err != 0) ngx_log_stderr(err,"CSocekt::ServerReconnectChildServer()中pthread_mutex_lock2()失败，返回的错误码为%d!",err);

            lpngx_connection_t p_Conn=*pSocketObj->m_childFreeServerConnectList.begin();
            ...
            //_sleeptime = _sleeptime+1000*1000*5;
            usleep(_sleeptime);
...
void CSocekt::addOneChildConnectionToList(lpngx_connection_t _pconn)
{
    CLock lock(&m_childserverListMutex);
    m_childServerConnectList.push_back(_pconn);
}
void CSocekt::removeAllChildConnectList()
{
