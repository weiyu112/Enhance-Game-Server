...
char g_configpath[256];

ngx_server_type g_servertype;
LogicManager* p_LogicManger;
//程序主入口函数----------------------------------
...
{   
    ...
    strcpy(g_configpath,g_os_argv[1]);
    //_path[strlen(g_os_argv[1])]=='\0';
    strcat(g_configpath,"/nginx.conf");
    //sprintf(_path,"%s%s",g_os_argv[1],"\0/nginx.conf");


    if(p_config->Load(g_configpath) == false) //把配置文件内容载入到内存            
    {   
        ngx_log_init();    //初始化日志
    ...
    g_servertype = (ngx_server_type)p_config->GetIntDefault("ServerType",1);
    ...
    _hostname = p_config->GetString("HostName");
    _username = p_config->GetString("UserName");
    _password = p_config->GetString("PassWord");
    _dbname = p_config->GetString("DBName");
    _port = p_config->GetIntDefault("DBPort",3306);//  
    if(_hostname
    &&_username
    &&_password
    &&_dbname)
    {
        //printf("hostname:%s,username:%s,password:%s,dbname:%s,dbport:%d;",_hostname, _username, _password, _dbname,_port);
        ntyConnPoolInitDataBase(_hostname,_username,_password,_dbname,_port);
        ngx_log_stderr(0,"mysql 数据库初始化成功");
        _mpool = (nConnPool* )malloc(sizeof(nConnPool));
        memset(_mpool,0,sizeof(nConnPool));
        //_mpool->max_num=0;
        _mpool = (nConnPool* )ntyConnPoolInitialize(_mpool);
        if(_mpool==NULL)
        {
            ngx_log_stderr(0,"mysql 数据库连接池初始化失败");
            exitcode = 1;
            goto lblexit;
        }
    }
    ...
    {
        ngx_log_stderr(0,"mysql 数据库初始化失败");
        exitcode = 1;
        goto lblexit;
    }
    if(g_servertype==GataServer)
    {
        p_LogicManger = new GatawayManager();
    }else if(g_servertype==LoginServer)
    {
        p_LogicManger = new LoginManager();
    }
    
    p_LogicManger->initAllInterface();
    //------------------------------------
    //(6)创建守护进程
...
void freeresource()
{
    ...
    if(p_LogicManger)
    {
        delete p_LogicManger;
        p_LogicManger = nullptr;
    }
}
