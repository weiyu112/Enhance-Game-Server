...
static void ngx_init_connect_childserver();

//变量声明
static u_char  master_process[] = "master process";
const char* g_servername;
//描述：创建worker子进程
void ngx_master_process_cycle()
{    
    CConfig *_config = CConfig::GetInstance();
    char _servername[256];
    g_servername = _config->GetString("ServerName");

    strcpy(_servername,g_servername);

    strcat(_servername,(const char*)master_process);
    ...
    size_t size;
    int    i;
    size = sizeof(_servername);  //注意我这里用的是sizeof，所以字符串末尾的\0是被计算进来了的
    size += g_argvneedmem;          //argv参数长度加进来    
    if(size < 1000) //长度小于这个，我才设置标题
    {
        char title[1000] = {0};
        strcpy(title,_servername); //"master process"
...
static void ngx_start_worker_processes(int threadnums)
{
    char _workername[256];
    strcpy(_workername,g_servername);
    strcat(_workername,"worker process");
    int i;
    for (i = 0; i < threadnums; i++)  //master进程在走这个循环，来创建若干个子进程
    {
        ngx_spawn_process(i,_workername);
    } //end for
    return;
}
