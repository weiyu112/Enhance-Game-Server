...
const char *hostName = "120.76.41.223";
...
static inline void ensureCapacity(nSnailsSet *snail, int i) {
	...
	if ((snail->columns[i].real_length > snail->Bind[i].buffer_length)) {
		...
		snail->columns[i].buffer = (char *)realloc(snail->columns[i].buffer, snail->columns[i].real_length + 1);
...
void *ntyMySqlConnInitialize(nShrimpConn *Conn) {
	...
	ntylog("hostname:%s,username:%s,password:%s,dbname:%s,dbport:%d;\n",hostName, userName, password, dbName,dbPort);
...
void *ntyShrimpNew(nShrimp *shrimp) {
	...
	shrimp->Conn = (nShrimpConn *)ntyMySqlConnInitialize(shrimp->Conn);
...
void ntyConnPoolInitDataBase(const char* _hostname ,const char* _username ,const char* _password,const char * _dbname ,unsigned int _port)
{
	//ntylog("hostname:%s,username:%s,password:%s,dbname:%s,dbport:%d;",_hostname, _username, _password, _dbname,_port);
	hostName = const_cast<char*>(_hostname);
	userName = const_cast<char*>(_username);
	password = const_cast<char*>(_password);
	dbName = const_cast<char*>(_dbname);
	dbPort = _port;
}
void *ntyConnPoolInitialize(nConnPool *Pool) {
	int i = 0;
	ntylog("this is ntyConnPoolInitialize\n");
	if (Pool == NULL) return NULL;

	if (Pool->max_num == 0) {
		ntylog("this is ntyConnPoolInitialize Pool->max_num:%d\n",Pool->max_num);
...
void ntyConnPoolRetConnection(nShrimp *shrimp) {
	...
	if (0 == cmpxchg(&shrimp->con_lock, 0, 1, WORD_WIDTH)) {
		nConnPool *Pool = (nConnPool *)shrimp->Pool;
...
void ntyConnPoolCheckConnection(nShrimp *shrimp) {
	...
	if (0 == cmpxchg(&shrimp->con_lock, 0, 1, WORD_WIDTH)) {
		...
		if (!ntyShrimpPing(shrimp->Conn)) { //connect failed
			...
			shrimp = (nShrimp *)ntyShrimpClear(shrimp);

			shrimp = (nShrimp *)ntyShrimpNew(shrimp);
		}
...
#else
	vsnprintf((char*)sql, MAX_SQL_BUFFER_SIZE, format, ap);
#endif

	return ntyShrimpExecuteQuery(shrimp, (const char*)sql, strlen((const char*)sql));
...
void ntyConnPoolExecute(nShrimp *shrimp, const char *format, ...) {
	...
	vsprintf((char*)sql, format, ap);

	int ret = ntyShrimpExecute(shrimp, (const char*)sql, strlen((const char*)sql));
...
#define NTY_DB_SELECT_NEW_ITEM			"SELECT U_NAME, U_GENDER FROM TBL_USER;" //41d20b131a90ac40-2860c1e3407b54e0
...
static int ntyQueryNewsSelect(void *self, const char* id) {
	
	nConnPool *pool = (nConnPool* )self;
	if (pool == NULL) return NTY_RESULT_BUSY;
	Shrimp con = ntyConnPoolGetConnection(pool);
	int ret = -1;
	
	SnailsSet r = ntyConnPoolExecuteQuery(con, NTY_DB_SELECT_NEW_ITEM);
	if (r != NULL) {
		while (ntyShrimpStmtResultSetNext(r)) {
			const char *name = ntyResultSetgetString(r, 1);
			const char *gender = ntyResultSetgetString(r, 2);

			ntylog(" name:%s, gender:%s\n", name, gender);
			ret = 0;
		}
	}
...
// 	ntyQueryNewsSelect(pool, "41d20b131a90ac40-2860c1e3407b54e0");
